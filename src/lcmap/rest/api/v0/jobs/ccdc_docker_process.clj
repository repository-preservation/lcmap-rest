(ns lcmap.rest.api.v0.jobs.ccdc-docker-process
  (:require [clojure.tools.logging :as log]
            [clojure.core.match :refer [match]]
            [ring.util.response :as ring]
            [compojure.core :refer [GET HEAD POST PUT context defroutes]]
            [lcmap.client.jobs.ccdc-docker]
...
(defn get-result-path
  [result-id]
  (format "%s/%s"
          lcmap.client.jobs.ccdc-docker/context
          result-id))
...
(defroutes routes
  (context lcmap.client.jobs.ccdc-docker/context []
    (GET "/" request
