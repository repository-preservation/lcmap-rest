(ns lcmap.rest.api.v0.jobs.ccdc-piped-processes
  (:require [clojure.tools.logging :as log]
            [clojure.core.match :refer [match]]
            [ring.util.response :as ring]
            [compojure.core :refer [GET HEAD POST PUT context defroutes]]
            [lcmap.client.jobs.ccdc-pipe]
...
(defn get-result-path
  [result-id]
  (format "%s/%s"
          lcmap.client.jobs.ccdc-pipe/context
          result-id))
...
(defroutes routes
  (context lcmap.client.jobs.ccdc-pipe/context []
    (GET "/" request
